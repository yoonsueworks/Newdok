  ...
  return (
    ...
    >
      <div className="flex justify-between mb-2">
        ...
        <div className="label-l text-neutralgray-600">{date}</div>
      </div>
      <div className="body-s text-neutralgray-700 mb-3">{sampleText}</div>
      <div
        className="input-01 text-neutralgray-700 flex gap-x-1"
        onClick={() => router.push(`/brandHome/${brandId}`)}
      >
        <div className="w-5 h-5 rounded-xl flex-shrink-0 relative border border-warmgray-30">
          <Image
            alt={brandName}
            src={imageURL}
            fill
